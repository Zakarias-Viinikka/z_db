...
pub fn create_fts5_triggers_sql_builder(
...
CREATE TRIGGER {update_trigger} AFTER UPDATE ON {source_table} BEGIN INSERT INTO {fts_table}({fts_table}, rowid, {column}) VALUES ('delete', old.rowid, old.{column}); INSERT INTO {fts_table}(rowid, {column}) VALUES (new.rowid, new.{column}); END;
CREATE TRIGGER {delete_trigger} AFTER DELETE ON {source_table} BEGIN INSERT INTO {fts_table}({fts_table}, rowid, {column}) VALUES ('delete', old.rowid, old.{column}); END;",
